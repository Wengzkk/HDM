...
def train_epoch(model: HDMModel, dataloader: DataLoader, optimizer: torch.optim.Optimizer, device: str):
    model.train_mode()
    loss_sum = 0

    for (data, mask, _, _) in dataloader:
        data, mask = data.to(device), mask.to(device)

        # Forward pass
        sd_output = model(data, mask=mask)

        # Compute loss
        loss = model.cal_loss(sd_output, mask)
...
def test_epoch(model: HDMModel, dataloader: DataLoader, device: str, is_vis, img_dir, class_name, cal_pro):
    model.eval_mode()
    ...
    for (data, mask, _, name) in dataloader:
        data, mask = data.to(device), mask.to(device)

        # Forward pass
        sd_output = model(data)

        # Calculate anomaly maps
        anomaly_maps = model.cal_am(sd_output)
        scores.extend(anomaly_maps)

        # Save inputs for visualization
        test_imgs.extend(data.cpu().numpy())
        gt_masks.extend(mask.cpu().numpy())
        names.extend(name)

    # Compute metrics
    img_roc_auc, per_pixel_rocauc, pro_auc_score, threshold = \
        metric_cal(np.array(scores), gt_masks, cal_pro=cal_pro)

    # Visualization
    if is_vis:
        plot_sample_cv2(names, test_imgs, {'SD': scores}, gt_masks, save_folder=img_dir)
        plot_anomaly_score_distributions({'SD': scores}, gt_masks, save_folder=img_dir, class_name=class_name)

    return {'i_roc': img_roc_auc * 100, 'p_roc': per_pixel_rocauc * 100, 'p_pro': pro_auc_score * 100}
...
def main(args):
    ...
    if not kwargs['pure_test']:  # train the model first
        ...
        epoch_bar = tqdm(range(kwargs['num_epochs']), desc=f"HDM:{kwargs['class_name']}")
